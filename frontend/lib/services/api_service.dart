...
class ApiService {
  ...
  static List<Itinerary> _getMockSearchResults(String org, String dst, double? budget, [Map<String, dynamic>? preferences]) {
    ...
    final isRegional = (org.toLowerCase().contains('kanpur') && dst.toLowerCase().contains('lucknow')) ||
        (org.toLowerCase().contains('lucknow') && dst.toLowerCase().contains('kanpur'));

    final trainClasses = preferences?['train_classes'] != null
        ? List<String>.from(preferences!['train_classes'])
        : <String>[];

    final isGeneral = trainClasses.any((c) => c.toLowerCase().contains('general') || c.toLowerCase().contains('unreserved'));
    final trainFare = isRegional ? (isGeneral ? 60.0 : 145.0) : 1300.0;
    final trainProvider = isRegional
        ? (isGeneral ? 'Jan Shatabdi Express (12059) (General / Unreserved)' : 'Superfast Express (12555) (Sleeper (SL))')
        : 'Superfast Express (12555) (Sleeper (SL))';
    final trainClassLabel = isRegional ? (isGeneral ? 'General / Unreserved' : 'Sleeper (SL)') : 'Sleeper (SL)';
    ...
    final rawList = [
      ...
      Itinerary(
        id: 2,
        type: 'Cheapest',
        totalPrice: trainFare,
        totalDuration: isRegional ? 1.2 : 14.0,
        carbonFootprint: isRegional ? 2.5 : 45.0,
        reliabilityScore: 92.0,
        delayProbability: 0.08,
        averageDelay: 15.0,
        comfortScore: isGeneral ? 5.5 : 7.5,
        aiExplanation: 'Direct express rail route from $org to $dst matching your IRCTC class preferences.',
        legs: [
          ItineraryLeg(
            transportType: 'Train',
            ...
            destination: dst,
            departureTime: '06:15',
            arrivalTime: isRegional ? '07:30' : '07:30',
            price: trainFare,
            duration: isRegional ? 1.25 : 13.5,
            delayProbability: 0.08,
            averageDelay: 15.0,
            refundability: 'Refundable',
            seatClass: trainClassLabel,
            carbonFootprint: isRegional ? 2.5 : 42.0,
            bookingLink: 'https://www.irctc.co.in',
            hiddenCosts: {'base_fare': trainFare},
          ),
        ],
        pricePrediction: PricePrediction(
          indicator: 'Buy Now',
          confidence: 90,
          explanation: 'Standard IRCTC fixed distance tariff.',
          historicalTrend: [trainFare, trainFare, trainFare],
        ),
        delayPrediction: DelayPrediction(
        ...
        explanationDetails: ExplanationDetails(
          whySelected: 'Lowest out-of-pocket IRCTC fare.',
          pros: ['Extremely affordable (₹${trainFare.toInt()} total)', 'Direct intercity train'],
          cons: ['Fixed departure timing'],
          tradeOffs: 'Optimal budget choice.',
          moneySaved: 500.0,
          timeSavedMins: 0,
        ),
      ),
